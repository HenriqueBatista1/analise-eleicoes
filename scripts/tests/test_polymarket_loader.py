...
def test_save_probability_records_persists_new_records(db_session):
    records = [
        probability_record(market_id="market-1", hour=10),
        probability_record(market_id="market-2", candidate_name="Candidate B", hour=11),
    ]
...
def test_save_probability_records_skips_records_already_saved_in_database(db_session):
    ...
    record_already_saved_in_database = probability_record(market_id="market-1", hour=10)
    new_record = probability_record(market_id="market-1", hour=11)
...
def test_save_probability_records_skips_duplicate_records_from_same_batch(db_session):
    new_record = probability_record(market_id="market-1", hour=10)
    same_record_repeated_in_batch = probability_record(market_id="market-1", hour=10)
